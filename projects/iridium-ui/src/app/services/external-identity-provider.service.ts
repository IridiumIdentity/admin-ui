...
import { CreateExternalIdentityProviderRequest } from '../components/dashboard/domain/create-external-identity-provider-request';
...
export class ExternalIdentityProviderService {
  ...
  create(formGroup: FormGroup, tenantId: string) {
    ...
    request.externalProviderTemplateId =
      formGroup.controls['externalProviderTemplateId'].value;
    ...
    return this.http.post<ApplicationCreateResponse>(
      environment.iridium.domain + `tenants/${tenantId}/external-providers`,
      request,
      options
    );
  }
  ...
  getAll(tenantId: string) {
    ...
    const options = { headers: headers };
    return this.http.get<ExternalIdentityProviderSummary[]>(
      environment.iridium.domain + `tenants/${tenantId}/external-providers`,
      options
    );
  }
  ...
  get(tenantId: string, externalProviderId: string) {
    ...
    const options = { headers: headers };
    return this.http.get<ExternalProviderResponse>(
      environment.iridium.domain +
        `tenants/${tenantId}/external-providers/${externalProviderId}`,
      options
    );
  }
